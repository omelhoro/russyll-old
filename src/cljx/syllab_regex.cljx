...
(defn cluster-find-1 [ word]
  (-- word concluster-pat #(++ "|" (map (fn [mod ] (++  (mod #+clj % #+cljs  [%1 %2 %3]))) [sm1 sm2 sm3 sm4 sm5]))))
(defn ^:export  syll-single-1 [word]
    (reduce #(%2 %1) 
            (.toLowerCase  word) 
            [orpho-single translit cluster-find-1])) 

(syll-single-1 "мужской")
...
(defn update-last-cluster [coll]
  (let [lcl (last coll)
        [vm  cm] (partition-by #(or (globals/all-vows %) (= % \*)) lcl)] 
    (assoc coll (dec (count coll)) (if (> (count cm) 1)  (map #(++ (% (++ vm) cm)) [sm4-1 sm5-1]) (++ lcl)))))  
    
  
(map (fn [w] (reduce 
               (fn [coll c] (cond (and (globals/all-vows c) (= (last coll) [\*])) (update-in coll  [(dec (count coll))] #(conj % c)) 
                                  (or (globals/all-vows c) (= \* c)) (conj (update-last-cluster coll) [c])
                                  (= (count coll) 0) (conj coll [c])
                                  :else (update-in coll [(dec (count coll))] #(conj % c))
                                  ))
                     [] (seq w))) ["мужск*ой" "смол*енск"])

(partition-by globals/all-vows "смоленск")